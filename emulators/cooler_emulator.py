...
global clientname, cooler_on
cooler_on = False
r = random.randrange(1, 10000000)
clientname = 'baby_cooler_' + str(r)
...
class ConnectionDock(QDockWidget):
    ...
    def __init__(self, mc):
        ...
        self.eSubscribeTopic.setText(topic_cooler_cmd)

        self.eStatus = QPushButton('COOLER: OFF', self)
        ...
        self.setWindowTitle('Cooler Emulator')
    ...
    def update_state(self, command):
        ...
        if command.strip() == 'ON':
            cooler_on = True
            self.eStatus.setText('COOLER: ON')
            self.eStatus.setStyleSheet('background-color: cyan; font-weight: bold;')
            self.mc.publish_to(topic_cooler_sts, 'ON')
            print('Cooler turned ON')
        elif command.strip() == 'OFF':
            cooler_on = False
            self.eStatus.setText('COOLER: OFF')
            self.eStatus.setStyleSheet('background-color: gray; font-weight: bold;')
            self.mc.publish_to(topic_cooler_sts, 'OFF')
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        self.setGeometry(30, 550, 320, 160)
        self.setWindowTitle('Cooler Emulator - Baby Room')
