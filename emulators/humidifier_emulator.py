...
global clientname, humidifier_on
humidifier_on = False
r = random.randrange(1, 10000000)
clientname = 'baby_humidifier_' + str(r)
...
class ConnectionDock(QDockWidget):
    ...
    def __init__(self, mc):
        ...
        self.eSubscribeTopic.setText(topic_humidifier_cmd)

        self.eStatus = QPushButton('HUMIDIFIER: OFF', self)
        ...
        self.setWindowTitle('Humidifier Emulator')
    ...
    def update_state(self, command):
        ...
        if command.strip() == 'ON':
            humidifier_on = True
            self.eStatus.setText('HUMIDIFIER: ON')
            self.eStatus.setStyleSheet('background-color: lightblue; font-weight: bold;')
            self.mc.publish_to(topic_humidifier_sts, 'ON')
            print('Humidifier turned ON')
        elif command.strip() == 'OFF':
            humidifier_on = False
            self.eStatus.setText('HUMIDIFIER: OFF')
            self.eStatus.setStyleSheet('background-color: gray; font-weight: bold;')
            self.mc.publish_to(topic_humidifier_sts, 'OFF')
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        self.setGeometry(30, 750, 320, 160)
        self.setWindowTitle('Humidifier Emulator - Baby Room')
